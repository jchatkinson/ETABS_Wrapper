...
namespace ETABSv1
{
    public static class cAreaObjExtension
    {
		...
		public static (int NumberGroups, string[] Groups)
		    GetGroupAssign(this cAreaObj obj, string Name) {
			int NumberGroups;
		...
		public static (int NumberPoints, double[] Offsets)
		    GetOffsets3(this cAreaObj obj, string Name) {
			int NumberPoints;
			NumberPoints = default(int);
			double[] Offsets;
			Offsets = default(double[]);
		    var res = obj.GetOffsets3(Name, ref NumberPoints, ref Offsets);
		    return (NumberPoints, Offsets);
		}
		public static (int NumEdges, int[] CurveType, double[] Tension, int[] NumPoints, double[] gx, double[] gy, double[] gz)
		    GetCurvedEdges(this cAreaObj obj, string Name) {
			int NumEdges;
			NumEdges = default(int);
			int[] CurveType, NumPoints;
			CurveType = NumPoints = default(int[]);
			double[] Tension, gx, gy, gz;
			Tension = gx = gy = gz = default(double[]);
		    var res = obj.GetCurvedEdges(Name, ref NumEdges, ref CurveType, ref Tension, ref NumPoints, ref gx, ref gy, ref gz);
		    return (NumEdges, CurveType, Tension, NumPoints, gx, gy, gz);
		}
