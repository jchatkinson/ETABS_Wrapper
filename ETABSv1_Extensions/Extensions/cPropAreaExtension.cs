...
namespace ETABSv1
{
    public static class cPropAreaExtension
    {
		...
		public static (int NumberLayers, string[] LayerName, double[] Dist, double[] Thickness, int[] MyType, int[] NumIntegrationPts, string[] MatProp, double[] MatAng, int[] MatBehavior, int[] S11Type, int[] S22Type, int[] S12Type)
		    GetShellLayer_2(this cPropArea obj, string Name) {
			int NumberLayers;
			NumberLayers = default(int);
			string[] LayerName, MatProp;
			LayerName = MatProp = default(string[]);
			double[] Dist, Thickness, MatAng;
			Dist = Thickness = MatAng = default(double[]);
			int[] MyType, NumIntegrationPts, MatBehavior, S11Type, S22Type, S12Type;
			MyType = NumIntegrationPts = MatBehavior = S11Type = S22Type = S12Type = default(int[]);
		    var res = obj.GetShellLayer_2(Name, ref NumberLayers, ref LayerName, ref Dist, ref Thickness, ref MyType, ref NumIntegrationPts, ref MatProp, ref MatAng, ref MatBehavior, ref S11Type, ref S22Type, ref S12Type);
		    return (NumberLayers, LayerName, Dist, Thickness, MyType, NumIntegrationPts, MatProp, MatAng, MatBehavior, S11Type, S22Type, S12Type);
		}
		public static int
		...
		public static (eDeckType DeckType, string SlabFillMatProp, string DeckMatProp, double SlabDepth, double RibDepth, double RibWidthTop, double RibWidthBot, double RibSpacing, double DeckShearThickness, double DeckUnitWeight, double ShearStudDia, double ShearStudHs, double ShearStudFu, int color, string notes, string GUID)
		    GetDeck_1(this cPropArea obj, string Name) {
			eDeckType DeckType;
			DeckType = default(eDeckType);
			string SlabFillMatProp, DeckMatProp, notes, GUID;
			SlabFillMatProp = DeckMatProp = notes = GUID = default(string);
			double SlabDepth, RibDepth, RibWidthTop, RibWidthBot, RibSpacing, DeckShearThickness, DeckUnitWeight, ShearStudDia, ShearStudHs, ShearStudFu;
			SlabDepth = RibDepth = RibWidthTop = RibWidthBot = RibSpacing = DeckShearThickness = DeckUnitWeight = ShearStudDia = ShearStudHs = ShearStudFu = default(double);
			int color;
			color = default(int);
		    var res = obj.GetDeck_1(Name, ref DeckType, ref SlabFillMatProp, ref DeckMatProp, ref SlabDepth, ref RibDepth, ref RibWidthTop, ref RibWidthBot, ref RibSpacing, ref DeckShearThickness, ref DeckUnitWeight, ref ShearStudDia, ref ShearStudHs, ref ShearStudFu, ref color, ref notes, ref GUID);
		    return (DeckType, SlabFillMatProp, DeckMatProp, SlabDepth, RibDepth, RibWidthTop, RibWidthBot, RibSpacing, DeckShearThickness, DeckUnitWeight, ShearStudDia, ShearStudHs, ShearStudFu, color, notes, GUID);
		}
